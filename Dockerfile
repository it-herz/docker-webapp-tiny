...
RUN	sed -i -e 's/v3\.2/edge/g' /etc/apk/repositories && \
    ...
    cd /root && git clone https://github.com/mongodb/mongo-php-driver.git && cd mongo-php-driver && git submodule sync && git submodule update --init && phpize && ./configure && make all -j 5 && make install && rm -r /root/mongo-php-driver && echo "extension=mongodb.so" > /usr/local/etc/php/conf.d/docker-php-ext-mongodb.ini && \
